...
void transform(const double * const target, const size_t tindex, const darray * const replacement, double * const dest, const size_t destindex){
	...
	fprintf(stderr, "hypotenuse: %f\n", hypotenuse);
	double angle = atan2(target[tindex+3] - target[tindex+1],  target[tindex+2] - target[tindex+0]);
	fprintf(stderr, "angle: %f\n", angle);
	double sine   = (target[tindex+3] - target[tindex+1]) / hypotenuse;
	double cosine = (target[tindex+2] - target[tindex+0]) / hypotenuse;
	fprintf(stderr, "sine: %f sin(): %f\n", sine, sin(angle));
	fprintf(stderr, "cosine: %f cos(): %f\n", cosine, cos(angle));
	
	fprintf(stderr, "Mapping rule onto (%f,%f) -- (%f,%f)\n", target[tindex], target[tindex+1], target[tindex+2], target[tindex+3]);
	...
	for (size_t k = 0; k < replacement->length; k += 2){
		...
		fprintf(stderr, "%zu %zu %f,%f\n", destindex+k, destindex+k+1, dest[destindex+k], dest[destindex+k+1]);
	}
}
...
darray* iteration(const darray * const list, const darray * const rule){
	...
	for (size_t i=0, j=2; i < list->length - 2; i += 2){
		...
		fprintf(stderr, "next[%zu]=current[%zu]\n", j, i+2);
		fprintf(stderr, "next[%zu]=current[%zu]\n", j+1, i+3);
		j += 2;
	}
	return newlist;
